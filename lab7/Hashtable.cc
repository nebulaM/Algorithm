...
void Hashtable::qinsert(int k) {
    ...
    // ************* ADD YOUR CODE HERE *******************
		int position=hash(k)%_size;
		for(int i=1;i<=_size;i++){
			if(_table[position]==EMPTY){
				_table[position]=k;
				tallyProbes(i);
				return;
			}else{
				position=(position+i*i)%_size;
			}
		}
...
void Hashtable::dinsert(int k) {
    ...
    // ************* ADD YOUR CODE HERE *******************
		int position=hash(k), inc=hash2(k);
		for(int i=1;i<=_size;i++){
			if(_table[position]==EMPTY){
				_table[position]=k;
				tallyProbes(i);
